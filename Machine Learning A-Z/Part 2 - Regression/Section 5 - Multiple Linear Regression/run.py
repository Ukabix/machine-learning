...
# Avoiding the dummy var trap manually - normally the library does this:
X = X[:, 1:]
# output:
...
# Fittting Multiple Linear Regression to the TRAINING set
...
# in order to use the above we need an x0-> add a column of 1s
# add X array to created by np.ones array of 1s, axis = 0 to get a line, not a col
